...
xybar_expansion = xybar(np.log(Expansion_Otto_Cycle['Avg p (kPa)']), np.log(
    Expansion_Otto_Cycle['vol (cc)']), 383)
xsqbar_expansion = avg_of_square(np.log(Expansion_Otto_Cycle['vol (cc)']))

a = (xybar_expansion - xbar_expansion*ybar_expansion) / \
    (xsqbar_expansion - xbar_expansion*xbar_expansion)
a0 = (ybar_expansion*xsqbar_expansion - xybar_expansion*xbar_expansion) / \
    (xsqbar_expansion - xbar_expansion*xbar_expansion)
m_AtoB_ = -1*a
...
ybar_compression = bar(
    np.log(Compression_Otto_Cycle_Regression['Avg p (kPa)']))
xybar_compression = xybar(np.log(Compression_Otto_Cycle_Regression['Avg p (kPa)']), np.log(
    Compression_Otto_Cycle_Regression['vol (cc)']), 251)
xsqbar_compression = avg_of_square(
    np.log(Compression_Otto_Cycle_Regression['vol (cc)']))

b = (xybar_compression - xbar_compression*ybar_compression) / \
    (xsqbar_compression - xbar_compression*xbar_compression)
b0 = (ybar_compression*xsqbar_compression - xybar_compression *
      xbar_compression)/(xsqbar_compression - xbar_compression*xbar_compression)
m_CtoD_ = -1*b
...
for i in range(len(Main_Otto_Cycle)-1):
    ...
    # Here, x2-x1 = dx = dV and (y1+y2)/2 = y = P
...
for i in range(len(Expansion_Otto_Cycle)-1):
    ...
    # print(x1, x2, y1, y2, ((x2-x1)*(y1+y2)*0.5))
    # Here, x2-x1 = dx = dV and (y1+y2)/2 = y = P
    area_expansion += ((x2-x1)*(y1+y2)*0.5)
...
for i in range(len(Compression_Otto_Cycle_Regression)-1):
    ...
    # print(x1, x2, y1, y2, ((x2-x1)*(y1+y2)*0.5))
    # Here, x2-x1 = dx = dV and (y1+y2)/2 = y = P
    area_compression += ((x2-x1)*(y1+y2)*0.5)
...
for i in range(len(Actual_Expansion)-1):
    ...
    # print(x1, x2, y1, y2, ((x2-x1)*(y1+y2)*0.5))
    # Here, x2-x1 = dx = dV and (y1+y2)/2 = y = P
    area_actual_expansion += ((x2-x1)*(y1+y2)*0.5)
...
for i in range(len(Extra_Compression)-1):
    ...
    # print(x1, x2, y1, y2, ((x2-x1)*(y1+y2)*0.5))
    # Here, x2-x1 = dx = dV and (y1+y2)/2 = y = P
    area_extra_compression += ((x2-x1)*(y1+y2)*0.5)

area_actual_compression = area_compression + area_extra_compression

print('\nWork Done during adiabatic compression from C to D (By area under the curve) =',
      round(area_compression/1000, 2), 'Joules')
print('Work Done during adiabatic expansion from A to B (By area under the curve) =',
      round(area_expansion/1000, 2), 'Joules')
...
print('Work Done (By area under the curve) =', round(area/1000, 2), 'Joules')
print('Total Work Done by in expansion and compression (By area under the curve) =',
...
Work_AtoB = (np.exp(
    a0 + a*np.log(Vvalues[1]))*Vvalues[1]-np.exp(a0+a*np.log(30.5))*30.5)/(1-m_AtoB_)
Work_CtoD = (np.exp(b0 + b*np.log(Vvalues[3]))*Vvalues[3] -
             np.exp(b0 + b*np.log(Vvalues[4]))*Vvalues[4])/(1-m_CtoD_)
print('\nWork Done during compression from E to D (By Formula) =',
      round(Work_CtoD/1000, 2))
print('Work Done during expansion from F to B (By Formula) =',
      round(Work_AtoB/1000, 2))
print('Work Done (By formula) =', round(
...
IMEP_Compression = -area_compression/Displacement_Volume
print('\nIMEP for compression process is', IMEP_Compression)
IMEP_Expansion = area_actual_expansion/Displacement_Volume
print('IMEP for expansion process is', IMEP_Expansion)
...
ax1.scatter(Vvalues, Pvalues, c=[
            'brown', 'green', 'black', 'purple', 'blue', 'magenta'], label='Data Points', s=100)
...
ax2.scatter(np.log(Vvalues), np.log(Pvalues), c=[
            'brown', 'green', 'black', 'purple', 'blue', 'magenta'], label='Data Points', s=100)
